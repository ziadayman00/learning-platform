    ...
    <div className="flex flex-col min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            {course.category && (
              <>
                <span>/</span>
                <Link href={`/courses?category=${course.category.slug}`} className="hover:text-black transition-colors">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-6 mb-8 text-gray-300">
                ...
                {course.averageRating > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{course.averageRating} ({course._count.reviews} reviews)</span>
                  </div>
                )}
                <div className="flex items-center gap-2">
                  <Users className="w-5 h-5" />
                  <span>{course._count.purchases.toLocaleString()} students</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <Button size="lg" variant="outline" className="h-14 px-8 text-lg rounded-full border-white hover:bg-white text-black">
                  ...
                  <div className="bg-white rounded-2xl p-8 border border-gray-200">
                    ...
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600 mb-2">{course.instructor._count.courses} courses</p>
                        <p className="text-gray-600">{course._count.purchases} total students</p>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="bg-white rounded-2xl p-8 border border-gray-200">
                      <div className="flex items-center gap-8 mb-8">
                        <div className="text-center">
                          <div className="text-6xl font-black mb-2">{course.averageRating}</div>
                          <div className="flex items-center justify-center gap-1 mb-2">
                            {renderStars(course.averageRating)}
                          </div>
                          <p className="text-gray-600">{course._count.reviews} reviews</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {course.reviews.length > 0 ? (
                    <div className="space-y-4">
                      {course.reviews.map((review) => (
                        <div key={review.id} className="bg-white rounded-2xl p-8 border border-gray-200">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl p-8 border border-gray-200 sticky top-24">
                <div className="text-center mb-6">
                  <div className="text-5xl font-black mb-2">${course.price}</div>
                  <p className="text-gray-600">One-time payment • Lifetime access</p>
                </div>
                <Button size="lg" className="w-full h-14 text-lg rounded-full mb-4">
                  Enroll Now
                  <ArrowRight className="ml-2 w-5 h-5" />
                </Button>
                ...
                <div className="border-t border-gray-200 pt-6 space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-700">{course._count.purchases} students enrolled</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <BookOpen className="w-5 h-5 text-gray-400" />
                    <span className="text-sm text-gray-700">All levels welcome</span>
                  </div>
                  {course.averageRating > 0 && (
                    <div className="flex items-center gap-3">
                      <Star className="w-5 h-5 fill-yellow-400 text-yellow-400" />
                      <span className="text-sm text-gray-700">{course.averageRating} average rating</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {relatedCourses.length > 0 && (
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-4xl font-bold mb-8">More in {course.category?.name}</h2>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      {relatedCourse.averageRating > 0 ? (
                        <div className="flex items-center gap-1 text-sm">
                          ...
                          <span className="font-medium">{relatedCourse.averageRating}</span>
                        </div>
                      ) : (
                        <div className="text-sm text-gray-400">No reviews</div>
                      )}
                      <div className="text-sm text-gray-600">{relatedCourse._count.purchases} students</div>
                    </div>
                    <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                      <div className="text-2xl font-black">${relatedCourse.price}</div>
